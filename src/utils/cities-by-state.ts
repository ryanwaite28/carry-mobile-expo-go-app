...
export const statesMap: PlainObject = {};

export const citiesByState: {
  [key:string]: { city: string, state: string }[]
} = {};
cities_states.forEach((c) => {
  statesMap[c.state] = c.state;

  const stateKey = c.state.replace(' ', '');
  if (!citiesByState[stateKey]) {
    citiesByState[stateKey] = [];
  }

  citiesByState[stateKey].push(c);
});

export const statesList: string[] = Object.keys(statesMap).sort();
export const statesSelectInputItems = statesList.map((s) => ({ key: s, label: s, value: s }));

export const getCitySelectInputItemsByState = (state) => {
  const stateKey = state.replace(' ', '');
  const results = citiesByState[stateKey]
    .map(c => c.city)
    .sort()
    .map((c) => ({ key: c, label: c, value: c }));
